...
with $\mu$ a parameter to be calibrated using `historical data`. Typically, we'll take $$\mu = \underbrace{\sigma\big( \text{mid\_price\_return} \big)}_{\text{volatility}} \sigma\big(spread  \big).$$
